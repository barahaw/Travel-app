...
export const geoNamesAPI = async (location) => {
  ...
  const getCoordinates = await fetch(
    geonames_baseURL + location + geonames_URL + goeNames_userName
  )
...
export const pixabayAPI = async (city) => {
  ...
  const url_city =
    pixabay_baseURL + pixabayAPI_KEY + "&q=" + city + pixabay_URL;
...
export function updateUI(trips) {
  ...
  const imgUrl = trips.pic ? trips.pic.src : "";
  const d_temp_high = trips.forecastData
    ? Math.round(trips.forecastData.daily_tempHigh)
    : "";
  const d_temp_low = trips.forecastData
    ? Math.round(trips.forecastData.daily_tempLow)
    : "";
  const temp = trips.forecastData
    ? Math.round(trips.forecastData.current_temp)
    : "";
  const d_icon = trips.forecastData ? trips.forecastData.daily_icon : "";
  const c_icon = trips.forecastData ? trips.forecastData.current_icon : "";
  ...
  if (trips.coords.country == undefined) {
    ...
    document.querySelector(".country").innerHTML =
      trips.coords.city + ", " + trips.coords.country;
  }
  ...
  if (countdownDays > 7) {
    ...
    document.querySelector(".temps").innerHTML =
      "High- " + d_temp_high + "&#8451;, Low- " + d_temp_low + "&#8451;";
